...
def chart_grouped_income_mix(yearly_rows):
    ...
    ax.bar([i - width / 2 for i in x], eenmalig, width=width, label="Eenmalig", color="#1F5D8B")
    ax.bar([i + width / 2 for i in x], periodiek, width=width, label="Periodiek", color="#5B8DB8")
...
def render_dashboard_tab(data):
    ...
    section_header("Financieel", "Kerncijfers voor resultaat, inkomsten, kas en uitgaven")
    ...
    section_header("Donateurs", "Kerncijfers voor omvang, activiteit en concentratie")
    ...
    with right:
        ...
        st.markdown(
            "<div class='summary'>"
            f"• Het netto resultaat komt uit op <strong>{eur(netto_incl_kas)}</strong> inclusief kas.<br>"
            "• De inkomstenbasis wordt hoofdzakelijk gedragen door <strong>eenmalige donaties</strong>.<br>"
            "• Strategisch blijft het belangrijk om het aandeel <strong>periodieke donaties</strong> verder te vergroten.<br>"
...
# OVERRIDE_IBAN_DONOR_DASHBOARD_V1
def render_dashboard_tab(data):
    meta = load_current_period_meta()
    fin = load_financial_summary()
    period_text = str(meta.get("period_label", "gekozen periode"))
    ...
    section_header("Management Dashboard", "Alle donor-aantallen op basis van 1 unieke IBAN = 1 donor-ID • " + period_text)
    ...
    section_header("Donateurs", "1 unieke IBAN = 1 donor-ID")
    d1, d2, d3, d4, d5 = st.columns(5)
    with d1:
        kpi_card("Aantal donateurs", i0(donor_count), "unieke IBAN's")
    with d2:
        kpi_card("Aantal transacties", i0(total_tx), "totaal aantal donaties")
    with d3:
    ...
    with right:
        ...
        st.markdown(
            ...
            f"• Netto resultaat inclusief kas: <strong>{eur(netto_incl_kas)}</strong><br>"
            f"• Banksaldo: <strong>{eur(banksaldo)}</strong><br>"
            f"• Donor-aantallen zijn gebaseerd op <strong>{i0(donor_count)}</strong> unieke IBAN's en <strong>{i0(total_tx)}</strong> transacties<br>"
            "• Contant in kas is apart opgenomen en wordt niet meegenomen in de grafieken."
            "</div>",
            unsafe_allow_html=True,
        )
...
def render_retention_tab(data):
    ...
    st.markdown(
        "<div class='info-box'>"
        "Deze tabel is gebaseerd op 1 unieke IBAN = 1 donor-ID. Eén IBAN telt als één donor, ook als die donor meerdere keren heeft gedoneerd. "
        f"De rij met laatste actieve jaar {current_year - 1} betreft een voorlopige risicogroep die in het huidige jaar nog kan terugkeren."
        "</div>",
        unsafe_allow_html=True,
    )
